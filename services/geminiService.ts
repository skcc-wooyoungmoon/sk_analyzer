...
- <b>호출 구조 (Call Structure):</b> 높은 수준의 호출 계층 또는 구성 요소 상호 작용 흐름을 설명하세요. **추가적으로, Mermaid.js 문법을 사용하여 시각적인 순서도를 생성하세요. 차트는 \`<div class="mermaid">\` 태그로 감싸야 합니다. 예시: \`<div class="mermaid">graph TD; A[클라이언트] --> B[API]; B --> C[데이터베이스];</div>\`**. 이렇게 하면 순서도로 렌더링됩니다.
